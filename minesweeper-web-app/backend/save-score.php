...
/**
 * POST /save-score.php
 * Body (JSON): { difficulty, score, time_taken, result, hints_used }
 *
...
// request body. This is the "reasonable score" check: since score is
// entirely determined by time_taken and hints_used, recalculating it
// here makes an inflated/faked score (e.g. edited in devtools)
// impossible rather than just "detected."
$score = max(0, 1000 - ($timeTaken * 5) - ($hintsUsed * 50));
...
$signature = md5("{$userId}|{$difficulty}|{$result}|{$timeTaken}|{$hintsUsed}");
...
$stmt = $pdo->prepare(
    'INSERT INTO game_results (user_id, difficulty, score, time_taken, result, hints_used)
     VALUES (?, ?, ?, ?, ?, ?)'
);
$stmt->execute([$userId, $difficulty, $score, $timeTaken, $result, $hintsUsed]);
